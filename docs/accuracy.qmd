...
#| fig-cap: "Maximum interpolation error for Runge's function. The b-series kernels converge at 7th order down to about 1e-15; \"a4\" matches a cubic spline at 4th order. Chebyshev interpolation, shown for reference, requires its own non-uniform sample points."
#| code-fold: true
...
All b-series kernels follow the dashed $\mathcal{O}(h^7)$ line until they reach the limit of
double-precision arithmetic, about $10^{-15}$, at around 1500 samples. The kernel `"a4"`
converges at 4th order, like the cubic spline. Chebyshev interpolation converges faster than any
...
#| fig-cap: "Maximum error of the first three derivatives of Runge's function. The b-series kernels lose about one order of convergence per derivative (dashed lines), and their rounding floor rises slowly. Chebyshev differentiation converges fastest but amplifies rounding by about n² per derivative order."
#| code-fold: true
...
Each derivative costs the b-series kernels about one order of convergence: 6th order for the first
derivative, 5th for the second and 4th for the third. Their rounding floor rises slowly with the
