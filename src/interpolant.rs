...
pub struct Interpolant1D {
    ...
    columns: usize,
    /// Values and derivatives: the column table of the requested order, as 4-wide vectors padded
    /// with zero columns to a multiple of four, each row B = ⌈K/4⌉ consecutive vectors (empty
    /// for :a0 and for integrals, whose tables live in `integral`)
    ...
    scale: f64,
    /// Integrals only: the tables and anchoring data of the integral axis (None otherwise)
...
impl Interpolant1D {
    ...
    fn new(
        ...
        let integral_order = if derivative < 0 { (-derivative) as usize } else { 0 };

        // The kernel: nearest neighbour, or the column table of the requested order (its column
        // count K for every order; its padded rows only for values and derivatives)
        let (nearest, columns, padded_rows) = if kernel == "a0" {
            ...
            let padded_rows = if integral_order > 0 { Vec::new() } else { pad_rows(table) };
            (false, table.columns, padded_rows)
        };
        ...
        // Integrals: the integral axis, and the left tail built once along it (one value per
        // coefficient, a stride of 1); nothing for values and derivatives
        let (integral, tails) = if integral_order > 0 {
            let axis = IntegralAxis::new(kernel, integral_order).map_err(|e| PyValueError::new_err(e))?;
            let tails = axis.extend_tails(&coefs, 1, coefs.len(), 1);
            (Some(axis), tails)
        } else {
            (None, Vec::new())
        };
        ...
        Ok(Interpolant1D {
            ...
            scale: if integral_order > 0 {
                h.powi(integral_order as i32)
            } else {
    ...
    fn evaluate<'py>(
        ...
        let values = if self.integral.is_some() {
...
impl Interpolant1D {
    ...
    /// Evaluate the m-fold integral at every point, for a kernel with K stencil columns stored as
    /// B 4-wide vectors per table row, as Julia's generic integral evaluator does in 1D:
    ///   in the stencil       — the anchored weights of the integral axis
    ...
    fn evaluate_integral<const K: usize, const B: usize>(
        ...
        let Some(axis) = self.integral.as_ref() else {
            return Err(PyValueError::new_err("integral evaluator called without integral data"));
        };
        let m = axis.order;
        let mut out: Vec<f64> = Vec::with_capacity(points.len());
        for &x in points.iter() {
            self.check_range(x)?;
            let (start, t) = self.locate(x);
            // the anchored weights of the K stencil coefficients
            let weights = axis.weights::<K, B>(start, t);
            // the K coefficients of the stencil, as a fixed-size array (one bounds check)
            let c: &[f64; K] = self.coefs[start..start + K].try_into().unwrap();
            let mut sum = 0.0_f64;
            for k in 0..K {
...
pub(crate) fn pad_values(rows: &[f64], k: usize) -> Vec<f64x4> {
    let blocks = (k + 3) / 4;
